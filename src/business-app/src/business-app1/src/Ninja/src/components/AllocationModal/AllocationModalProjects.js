...
const AllocationModalProjects = ({
  ...
  useEffect(() => {
    const fetchEmployeesInitial = async () => {
      try {
        const response = await axios.get('http://localhost:7071/api/employees/search?query=');
  ...
  useEffect(() => {
    const fetchTotalAllocation = async () => {
      ...
      try {
        const response = await axios.get(`http://localhost:7071/api/employee-allocations/${formData.employeeId}`, {
          params: {
  ...
  useEffect(() => {
    const fetchModalData = async () => {
      ...
      setFetchError(null);
      try {
        const response = await axios.get('http://localhost:7071/api/modal/data');
    ...
    const fetchEmployeeDetails = async () => {
      try {
        ...
        if (formData.employeeId && !formData.employeeName) {
          response = await axios.get(`http://localhost:7071/api/employees/${formData.employeeId}`);
          setFormData(prev => ({
            ...prev,
            employeeName: response.data.employeeName || '',
          }));
        } else if (formData.employeeName && !formData.employeeId) {
          response = await axios.get(`http://localhost:7071/api/employees/search?query=${formData.employeeName}`);
          setFormData(prev => ({
            ...prev,
  ...
  const fetchEmployees = useCallback(_.debounce(async (query, type) => {
    ...
    try {
      const response = await axios.get('http://localhost:7071/api/employees/search', {
        params: { query }
      });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const overlapResponse = await axios.get(`http://localhost:7071/api/employee-details/${formData.employeeId}/allocations`);
      ...
      if (allocationData) {
        ...
        const totalAllocationResponse = await axios.get(`http://localhost:7071/api/employee-allocations/${formData.employeeId}`, {
          params: {
            startDate: formData.startDate,
            endDate: formData.endDate
          }
      ...
      if (allocationData && allocationData.AllocationID) {
        // Editing existing allocation
        await axios.put(`http://localhost:7071/api/allocations/${allocationData.AllocationID}`, payload);
        toast.success('Allocation updated successfully!');
      } else {
        // Adding new allocation
        await axios.post('http://localhost:7071/api/api/allocate', payload);
