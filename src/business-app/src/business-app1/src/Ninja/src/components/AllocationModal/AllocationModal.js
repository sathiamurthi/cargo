  ...
  useEffect(() => {
    const fetchEmployeeDetails = async () => {
      if (formData.employeeId && !employeeData) {
        try {
          const response = await axios.get(`http://localhost:7071/api/employees/${formData.employeeId}`);
          if (response.data) {
            setFormData(prev => ({
              ...prev,
    ...
    const fetchEmployeeId = async () => {
      if (formData.employeeName && !employeeData) {
        try {
          const response = await axios.get(`http://localhost:7071/api/employees?name=${formData.employeeName}`);
