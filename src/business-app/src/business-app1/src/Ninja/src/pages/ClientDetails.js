...
const ClientDetails = () => {
  ...
  const fetchAllocations = async (currentFilter) => {
    ...
    setError(null);
    try {
      const response = await fetch(`http://localhost:7071/api/project-details/${clientId}/${projectId}?filter=${currentFilter}`);
      if (response.ok) {
  ...
  const downloadExcel = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:7071/api/project-details-all/${clientId}/${projectId}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const confirmDeleteAllocation = async () => {
    try {
      const response = await fetch(`http://localhost:7071/api/allocations/${allocationToDelete}`, {
        method: 'DELETE',
      });
      if (response.ok) {
        setShowMessage(true);
