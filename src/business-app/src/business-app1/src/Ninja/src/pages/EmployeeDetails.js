...
import { Icon, Table, Button, Popup, Message, Modal, Sidebar } from 'semantic-ui-react';
...
const EmployeeDetails = () => {  // Accept userRole as a prop
  const { userRole } = 'bizops'//React.useContext(UserContext);
  ...
  const fetchEmployeeData = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`http://localhost:7071/api/employee-details/${id}`);
  ...
  const fetchAllocations = async (currentFilter) => {
    try {
      setLoading(true);
      const response = await axios.get(`http://localhost:7071/api/employee-details/${id}/allocations`, {
  ...
  const handleDeleteAllocation = async (allocationId) => {
    try {
      await axios.delete(`http://localhost:7071/api/allocations/${allocationId}`);
  ...
  const handleDownloadExcel = async () => {
    ...
    try {
      ...
      const fetchPromises = filtersList.map(filterType =>
        axios.get(`http://localhost:7071/api/employee-details/${id}/allocations`, {
    ...
    <div className="main-layout">
      ...
      <div className='right-content'>
        <div className='breadcrumb'>
          ...
          <Icon name='close' onClick={confirmDiscardChanges} />
                  ...
                  {allocations && allocations.length > 0 ? (
                    allocations.map((alloc) => (
                      <Table.Row key={alloc.AllocationID || `new-${alloc.ProjectID}-${alloc.EmployeeID}`}>
                        ...
                        <Table.Cell>${alloc.AllocationBillingRate}</Table.Cell>
