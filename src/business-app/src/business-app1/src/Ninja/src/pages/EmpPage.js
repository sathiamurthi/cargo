...
const EmpPage = () => {
  ...
  const fetchDataBasedOnFilter = async (filter) => {
    try {
      ...
      switch (filter) {
        case 'unallocated':
          endpoint = 'http://localhost:7071/api/employees/unallocated';
          break;
        case 'draft':
          endpoint = 'http://localhost:7071/api/employees/draft';
          break;
        case 'allocated':
          endpoint = 'http://localhost:7071/api/employees/allocated';
          break;
        case 'bench':
          endpoint = 'http://localhost:7071/api/employees/bench';
          break;
        case 'all':
        default:
          endpoint = 'http://localhost:7071/api/employees';
          break;
      }
  ...
  const downloadExcel = async () => {
    ...
    try {
      for (const filter of filters) {
        const endpoint = `http://localhost:7071/api/employees${filter === 'all' ? '' : `/${filter}`}`;
        const response = await fetch(endpoint);
        if (!response.ok) {
